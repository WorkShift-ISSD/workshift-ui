...
const getCardsData = () => {
    if (tipoInforme === "licencias") {
      return [
        ...
        {
          label: "Días totales",
          value: estadisticas?.diasTotal ?? 0,
          color: [139, 92, 246],
        },
      ];
    }
    if (tipoInforme === "sanciones") {
      return [
        {
        ...
        {
          label: "Anuladas",
          value: estadisticas?.anuladas ?? 0,
          color: [107, 114, 128],
        },
      ];
    }
    if (tipoInforme === "cambios-turno") {
      const total = estadisticas?.total ?? 0;
      const aprobados = estadisticas?.aprobados ?? 0;
      const rechazados = estadisticas?.rechazados ?? 0;
      const tasaAprobacion =
        aprobados + rechazados > 0
          ? Math.round((aprobados / (aprobados + rechazados)) * 100)
          : 0;
      return [
        { label: "Total solicitudes", value: total, color: [37, 99, 235] },
        { label: "Aprobados", value: aprobados, color: [34, 197, 94] },
        { label: "Rechazados", value: rechazados, color: [239, 68, 68] },
        {
    ...
    return [
      ...
      {
        label: "Total Faltas",
        value: estadisticas.totalFaltas,
        color: [239, 68, 68],
      },
      {
        label: "Justificadas",
  ...
  const exportToPDF = async () => {
    try {
      ...
      const drawHeader = () => {
        ...
        if (logoMigraciones)
          doc.addImage(logoMigraciones, "PNG", 12, 7, 22, 22);

        doc.setFont("helvetica", "bold");
        doc.setFontSize(18);
        doc.setTextColor(255, 255, 255);
        doc.text(
          "Dirección Nacional de Migraciones - WSMS",
          pageWidth / 2,
          17,
          { align: "center" },
        );
      ...
      if (
        ...
        if (filtros.rol && filtros.rol !== "TODOS")
          filtrosAplicados.push(`Rol: ${filtros.rol}`);
        if (filtros.turno && filtros.turno !== "TODOS")
          filtrosAplicados.push(`Turno: ${filtros.turno}`);
        if (filtros.empleado && filtros.empleado !== "TODOS")
          filtrosAplicados.push("Empleado específico");
      ...
      cards.forEach((card, i) => {
        ...
        doc.setFontSize(14);
        doc.setTextColor(255, 255, 255);
        doc.text(String(card.value), x + cardWidth / 2, y + 10, {
          align: "center",
        });
        doc.setFontSize(8);
        doc.setFont("helvetica", "normal");
      ...
      switch (tipoInforme) {
        ...
        case "individual":
          ...
          break;
        case "cambios-turno":
          y = await drawCambiosTurno(doc, y, pageWidth, pageHeight, drawHeader);
          break;
case "sanciones":
          y = await drawSanciones(doc, y, pageWidth, pageHeight, drawHeader);
          break;
        case "licencias":
          y = await drawLicencias(doc, y, pageWidth, pageHeight, drawHeader);
          break;
      }
  ...
  const drawAsistencia = async (
    ...
    const colWidth = [13, 42, 16, 11, 18, 12, 14, 12, 14, 18];
    ...
    for (let dato of datos) {
      ...
      const row = [
        ...
        dato.nombre.length > 25
          ? dato.nombre.substring(0, 22) + "..."
          : dato.nombre,
        dato.rol,
        dato.turno,
        dato.diasDebioTrabajar.toString(),
        dato.faltas.toString(),
        (dato.licencias ?? 0).toString(),
        dato.faltasInjustificadas.toString(),
        (dato.sanciones ?? 0).toString(),
  ...
  const drawAusentismo = async (
    ...
    for (let dato of datos.porRol) {
      ...
      const nivel =
        promedio >= 5 ? "Crítico" : promedio >= 3 ? "Moderado" : "Bajo";
      const colorNivel =
        promedio >= 5
          ? [239, 68, 68]
          : promedio >= 3
...
const drawComparativo = async (
    ...
    doc.setFont("helvetica", "normal");
    doc.setTextColor(60, 60, 60);
    datos.comparacion.forEach((dato: any) => {
      if (dato.filtrosTexto) {
        doc.text(
          `Grupo ${dato.grupo}: ${dato.filtrosTexto}`,
          pageWidth / 2,
          y,
...
const headers = [
    ...
    const colWidth = [26, 26, 30, 26, 26, 30];
...
const row = [
  ...
  const drawIndividual = async (
    ...
    doc.setTextColor(60, 60, 60);
    doc.text(
      `Legajo: ${datos.empleado.legajo} | ${datos.empleado.rol} | Grupo ${datos.empleado.grupoTurno}`,
      15,
      y,
    );
    y += 10;
...
const metricas = [
      ...
      { label: "Licencias (días)", value: datos.licencias || 0 },
      { label: "Injustificadas", value: datos.faltasInjustificadas || 0 },
      { label: "Sanciones (días)", value: datos.sanciones || 0 },
      { label: "Días Trabajados", value: datos.diasTrabajados || 0 },
      { label: "% Asistencia", value: (datos.porcentajeAsistencia || 0) + "%" },
    ];

    const metricWidth = (pageWidth - 40) / metricas.length;
    metricas.forEach((m, i) => {
      ...
      doc.setFontSize(11);
      doc.setTextColor(31, 41, 55);
      doc.text(m.value.toString(), x + metricWidth / 2, y + 7, {
        align: "center",
      });
      doc.setFontSize(7);
      doc.setFont("helvetica", "normal");
    ...
    if (datos.detallesFaltas && datos.detallesFaltas.length > 0) {
      ...
      for (let falta of datos.detallesFaltas) {
        ...
        const motivo =
          (falta.causa || falta.motivo || "").length > 30
        ...
        const obs = falta.observaciones
          ? falta.observaciones.length > 35
            ? falta.observaciones.substring(0, 32) + "..."
            : falta.observaciones
          : "-";
  ...
  const drawCambiosTurno = async (
    ...
    const colWidth = [30, 18, 18, 20, 20, 50, 24];
    ...
    for (let dato of filas) {
      ...
      row.forEach((text, i) => {
        ...
        if (i === 4) {
          if (dato.estado === "APROBADA") doc.setTextColor(34, 197, 94);
          else if (dato.estado === "RECHAZADA") doc.setTextColor(239, 68, 68);
  ...
  const drawSanciones = async (
    ...
    const headers = ["Empleado", "Motivo", "Desde", "Hasta", "Estado"];
    const colWidth = [40, 60, 26, 26, 26];
    ...
    for (let dato of filas) {
      ...
      row.forEach((text, i) => {
        ...
        if (i === 4) {
          if (dato.estado === "ACTIVA") doc.setTextColor(239, 68, 68);
          else if (dato.estado === "FINALIZADA") doc.setTextColor(34, 197, 94);
  ...
  const drawLicencias = async (
    ...
    const colWidth = [42, 30, 26, 26, 18, 28];
    ...
    for (let dato of filas) {
      ...
      row.forEach((text, i) => {
        ...
        if (i === 5) {
          if (dato.estado === "ACTIVA") doc.setTextColor(34, 197, 94);
          else if (dato.estado === "PENDIENTE") doc.setTextColor(234, 179, 8);
          else if (dato.estado === "RECHAZADA") doc.setTextColor(239, 68, 68);
          else doc.setTextColor(107, 114, 128);
          doc.setFont("helvetica", "bold");
        }
  ...
  const exportToExcel = () => {
    ...
    const horaGen = now.toLocaleTimeString("es-AR", {
      hour: "2-digit",
      minute: "2-digit",
    });
    wsData.push([
      `Generado: ${fechaGen} ${horaGen} | Usuario: ${usuarioNombre}`,
    ]);
    wsData.push([]);
...
wsData.push([
    ...
    if (tipoInforme === "comparativo" && datos?.comparacion) {
      datos.comparacion.forEach((d: any) => {
        if (d.filtrosTexto) {
          wsData.push([`Grupo ${d.grupo}: ${d.filtrosTexto}`]);
        }
      });
      wsData.push([]);
    }
...
if (tipoInforme === "cambios-turno") {
...
} else if (tipoInforme === "sanciones") {
      wsData.push(["", "", "TOTAL", "ACTIVAS", "FINALIZADAS", "ANULADAS"]);
      wsData.push([
        "",
        "",
        estadisticas?.total ?? 0,
    ...
    } else if (tipoInforme === "licencias") {
      wsData.push(["", "", "TOTAL", "ACTIVAS", "PENDIENTES", "DÍAS TOTALES"]);
      wsData.push([
        "",
        "",
        estadisticas?.total ?? 0,
...
case "sanciones":
        wsData.push(["EMPLEADO", "MOTIVO", "DESDE", "HASTA", "ESTADO"]);
        (datos?.tabla || []).forEach((d: any) => {
      ...
      case "individual":
        if (datos?.empleado) {
          wsData.push([
            `EMPLEADO: ${datos.empleado.nombre} ${datos.empleado.apellido}`,
          ]);
          wsData.push([
            `LEGAJO: ${datos.empleado.legajo} | ${datos.empleado.rol} | Grupo ${datos.empleado.grupoTurno}`,
          ]);
...
wsData.push([
          ...
          wsData.push([
            datos.faltas || 0,
            datos.licencias || 0,
            datos.faltasInjustificadas || 0,
            datos.sanciones || 0,
            datos.diasTrabajados || 0,
            (datos.porcentajeAsistencia || 0) + "%",
          ]);
  ...
  return (
    <div ref={dropdownRef} className="relative inline-block text-left">
      <button
        ...
        className={
          className ||
          "flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors shadow-sm"
        }
      >
