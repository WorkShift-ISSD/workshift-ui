...
function validarFila(fila: any): FilaImport {
  ...
  if (!tipo) {
    ...
    errores.push(`tipo inválido (${TIPOS_VALIDOS.join(', ')})`);
  }
  ...
  if (tipo === 'COMPENSATORIO' && fila.fecha_desde !== fila.fecha_hasta) {
    errores.push('COMPENSATORIO es de un solo día (fecha_desde = fecha_hasta)');
  }
...
export default function ImportarLicenciasModal({ onClose, onSuccess }: Props) {
  ...
  const handleImportar = async () => {
    ...
    setCargando(true);
    try {
      const res = await fetch('/api/licencias/bulk', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
        body: JSON.stringify({ licencias: filasValidas }),
      });
  ...
  const descargarTemplate = () => {
    const ws = XLSX.utils.aoa_to_sheet([
      ['legajo', 'tipo', 'fecha_desde', 'fecha_hasta', 'observaciones'],
      [12345, 'ORDINARIA', '2026-07-01', '2026-07-05', 'Vacaciones'],
      [67890, 'MEDICA', '2026-06-20', '2026-06-22', ''],
      [11111, 'COMPENSATORIO', '2026-07-10', '2026-07-10', ''],
    ]);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Licencias');
    XLSX.writeFile(wb, 'template_licencias.xlsx');
  };
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto">
        <div className="px-6 py-4 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100">Importar Licencias desde Excel</h2>
          </div>
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-2 p-4 bg-green-50 dark:bg-green-900/20 rounded-lg">
                ...
                <p className="text-green-700 dark:text-green-400 font-medium">
                  {resultado.creadas} licencia{resultado.creadas !== 1 ? 's' : ''} importada{resultado.creadas !== 1 ? 's' : ''} correctamente
                </p>
              </div>
            ...
            <>
              <div className="flex items-start gap-2 p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
                ...
                <p className="text-sm text-blue-700 dark:text-blue-400">
                  Todas las licencias se importan como <strong>APROBADAS</strong>. Tipos válidos: {TIPOS_VALIDOS.join(', ')}
                </p>
              </div>
                  ...
                  <div className="overflow-x-auto rounded-lg border border-gray-200 dark:border-gray-700">
                    <table className="w-full text-sm">
                      <thead className="bg-gray-50 dark:bg-gray-700">
                        <tr>
                          {['legajo', 'tipo', 'fecha desde', 'fecha hasta', 'observaciones', 'validación'].map((col) => (
                      ...
                      <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                        {filas.map((f, i) => (
                          <tr key={i} className={f._valida ? '' : 'bg-red-50 dark:bg-red-900/10'}>
                            ...
                            <td className="px-3 py-2 text-gray-900 dark:text-gray-100">{f.tipo}</td>
                            ...
                            <td className="px-3 py-2 text-gray-500 dark:text-gray-400 max-w-[180px] truncate">{f.observaciones || '-'}</td>
                  ...
                  <div className="flex gap-3 pt-2">
                    ...
                    >
                      {cargando ? (
                        ...
                        `Importar ${filasValidas.length} licencia${filasValidas.length !== 1 ? 's' : ''}`
                      )}
                    </button>
                  </div>
                </div>
