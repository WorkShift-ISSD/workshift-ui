...
function validarFila(fila: any): FilaImport {
  ...
  } else if (!FECHA_REGEX.test(fila.fecha.toString())) {
    errores.push('fecha debe ser YYYY-MM-DD');
  }
  if (!fila.motivo) errores.push('motivo requerido');

  return {
    legajo: Number(fila.legajo),
    fecha: fila.fecha?.toString().trim() || '',
    motivo: fila.motivo?.toString().trim() || '',
    observaciones: fila.observaciones?.toString().trim() || undefined,
    justificada: fila.justificada?.toString().trim() || undefined,
...
export default function ImportarFaltasModal({ onClose, onSuccess }: Props) {
  ...
  const handleImportar = async () => {
    ...
    setCargando(true);
    try {
      const res = await fetch('/api/faltas/bulk', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
        body: JSON.stringify({ faltas: filasValidas }),
      });
  ...
  const descargarTemplate = () => {
    const ws = XLSX.utils.aoa_to_sheet([
      ['legajo', 'fecha', 'motivo', 'observaciones', 'justificada'],
      [12345, '2026-06-15', 'Inasistencia injustificada', '', 'NO'],
      [67890, '2026-06-15', 'Enfermedad', 'Con certificado', 'SI'],
    ]);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Faltas');
    XLSX.writeFile(wb, 'template_faltas.xlsx');
  };
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto">
        <div className="px-6 py-4 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100">Importar Faltas desde Excel</h2>
          </div>
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-2 p-4 bg-green-50 dark:bg-green-900/20 rounded-lg">
                ...
                <p className="text-green-700 dark:text-green-400 font-medium">
                  {resultado.creadas} falta{resultado.creadas !== 1 ? 's' : ''} importada{resultado.creadas !== 1 ? 's' : ''} correctamente
                </p>
              </div>
                  ...
                  <div className="overflow-x-auto rounded-lg border border-gray-200 dark:border-gray-700">
                    <table className="w-full text-sm">
                      <thead className="bg-gray-50 dark:bg-gray-700">
                        <tr>
                          {['legajo', 'fecha', 'motivo', 'observaciones', 'justificada', 'estado'].map((col) => (
                      ...
                      <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                        {filas.map((f, i) => (
                          <tr key={i} className={f._valida ? '' : 'bg-red-50 dark:bg-red-900/10'}>
                            ...
                            <td className="px-3 py-2 text-gray-900 dark:text-gray-100">{f.fecha}</td>
                            <td className="px-3 py-2 text-gray-900 dark:text-gray-100 max-w-[200px] truncate">{f.motivo}</td>
                            <td className="px-3 py-2 text-gray-500 dark:text-gray-400 max-w-[150px] truncate">{f.observaciones || '-'}</td>
                            <td className="px-3 py-2 text-gray-900 dark:text-gray-100">{f.justificada || 'NO'}</td>
                  ...
                  <div className="flex gap-3 pt-2">
                    ...
                    >
                      {cargando ? (
                        ...
                        `Importar ${filasValidas.length} falta${filasValidas.length !== 1 ? 's' : ''}`
                      )}
                    </button>
                  </div>
                </div>
