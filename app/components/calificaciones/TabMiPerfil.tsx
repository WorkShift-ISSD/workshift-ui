  ...
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        <div className="bg-white dark:bg-gray-800 rounded-xl p-5 border border-gray-400 dark:border-gray-700/50 flex flex-col items-center justify-center gap-2">
          <p className="text-gray-500 dark:text-gray-500 text-xs">Mi score general</p>
          ...
          <p className="text-gray-400 dark:text-gray-600 text-xs">{recibidas.length} calificaciones recibidas</p>
        </div>

        <div className="md:col-span-2 bg-white dark:bg-gray-800 rounded-xl p-5 border border-gray-400 dark:border-gray-700/50 space-y-3">
          <p className="text-gray-500 dark:text-gray-500 text-xs font-medium uppercase tracking-wider mb-1">Desglose por criterio</p>
          {criteriosPerfil.map((c) => (
            <div key={c.label} className="flex items-center gap-3">
              <span className="text-gray-500 dark:text-gray-400 text-xs w-44 flex-shrink-0">{c.label}</span>
              <div className="flex-1 bg-gray-200 dark:bg-gray-900 rounded-full h-1.5 overflow-hidden">
                <div className="bg-blue-500 h-1.5 rounded-full" style={{ width: `${c.pct}%` }} />
              </div>
              <span className="text-gray-700 dark:text-gray-300 text-xs w-7 text-right">{Number(c.valor).toFixed(1)}</span>
            </div>
          ))}
          <div className="border-t border-gray-400 dark:border-gray-700/50 pt-3 flex items-center gap-3">
            <span className="text-gray-500 dark:text-gray-500 text-xs w-44 flex-shrink-0">Cumplimiento (Sí/No)</span>
            <span className="text-xs">
              <span className="text-green-500 dark:text-green-400">{cumplSi} Sí</span>
              <span className="text-gray-400 dark:text-gray-600"> · </span>
              <span className="text-red-500 dark:text-red-400">{cumplNo} No</span>
            </span>
          </div>
        </div>
      </div>
      ...
      <div>
        ...
        {historial.length === 0 ? (
          <p className="text-gray-500 dark:text-gray-500 text-sm text-center py-8">No hay calificaciones aún.</p>
        ) : (
          <div className="space-y-2">
            {historial.map((item) => (
              <div key={item.id} className="bg-white dark:bg-gray-800 border border-gray-400 dark:border-gray-700/50 rounded-xl p-4 flex flex-col sm:flex-row items-start sm:items-center gap-3">
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  ...
                  <span className={`text-xs px-2 py-0.5 rounded ${item.cumplimiento ? 'text-green-500 dark:text-green-400' : 'text-red-500 dark:text-red-400'}`}>
