...
function EstadoBadge({ estado }: { estado: string }) {
  ...
  return (
    <span
      className={`px-2.5 py-0.5 rounded-full text-xs font-medium ${map[estado] ?? "bg-gray-100 text-gray-600"}`}
    >
      {estado}
    </span>
  );
...
type LicenciasExportData = {
  data: { tabla: any[]; stats: any };
  filtros: { desde: string; hasta: string; empleadoId: string; tipo: string };
};

export function InformeLicencias({ onDataChange }: { onDataChange?: (payload: LicenciasExportData) => void } = {}) {
...
const porTipo = useMemo(() => {
  ...
  useEffect(() => {
    onDataChange?.({
      ...
      filtros: { desde, hasta, empleadoId, tipo },
    });
  }, [tablaExport, stats, desde, hasta, empleadoId, tipo, onDataChange]);
    ...
    await generarPDF(
      {
        ...
        filename: `Licencias_${fechaArchivo}.pdf`,
      },
      [
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
          ...
          <div className="flex flex-col gap-1">
            ...
            >
              ...
              {(empleados || [])
                .filter(
                  (e: any) => e.rol !== "ADMINISTRADOR" && e.rol !== "JEFE",
                )
                .map((e: any) => (
        ...
        <div className="md:col-span-2 bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
          <div className="p-4 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <button
                onClick={exportarPDF}
                className="flex items-center gap-1.5 px-3 py-1.5 border border-gray-300 dark:border-gray-600 text-gray-600 dark:text-gray-300 text-xs rounded-lg hover:border-gray-400 transition-all"
              >
                ...
                <thead>
                  <tr className="border-b border-gray-200 dark:border-gray-700">
                    {[
                      ...
                      <th
                        key={h}
                        className="text-left px-4 py-3 text-gray-500 dark:text-gray-400 font-medium"
                      >
                        {h}
                      </th>
                    ))}
